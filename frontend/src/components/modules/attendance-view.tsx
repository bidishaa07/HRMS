...
export function AttendanceView({ user }: { user: User }) {
  ...
  const attendanceQuery = useQuery({ queryKey: ["attendance", month], queryFn: () => api.attendance(month) });
  useRealtime();
  const rows: Attendance[] = attendanceQuery.data ?? [];
  async function action(kind: "in" | "out") {
    setMessage("");
    try {
      const result = kind === "in" ? await api.checkIn() : await api.checkOut();
