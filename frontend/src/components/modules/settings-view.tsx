...
export function SettingsView({ user, onUserChange }: { user: User; onUserChange: (user: User) => void }) {
  ...
  async function changePassword(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api.changePassword({ current_password: String(data.get("current_password")), new_password: String(data.get("new_password")) }); setMessage("Password changed. Sign in again to start a new session."); form.reset(); }
